...
### Added
- Added instruction-only handoff planning trigger to `AGENTS.md` (and synced copies) at `plan/HANDOFF.md`, firing whenever the content digest differs from the session's acknowledged digest; pre-consent verification restricted to `git --no-pager branch` and `git --no-pager --no-lazy-fetch log -n 1 --format=%H --no-show-signature` only.
...
- Added `scripts/check_conflict_markers.py` with `--staged` mode reading index blobs without applying `working-tree-encoding`, object size pre-checking via `git cat-file -s` before buffering, fail-closed `git check-attr` error handling, balanced conflict block recognition across all positive marker widths ($N \ge 1$), single-fd file I/O via `_safe_read`, which closes the check-then-open window where `O_NOFOLLOW` exists and narrows it elsewhere, repo root resolution via `git rev-parse --show-toplevel`, UTF-16/32 BOM decoding, and Markdown Setext heading context validation.
- Added `tests/test_check_conflict_markers.py`, 46 tests covering 2-way and diff3 conflict blocks, marker widths 1 through 10, UTF-16 BOMs, symlinks, `_safe_read` unit tests, staged index decoding, oversized blob rejection without buffering, check-attr error propagation, Setext headings, CLI from subdirectory, `--staged` mode, and exact wiring across CI, Makefile, and pre-commit.
...
- Excluded skip-worktree entries from the tracked-file listing. A sparse checkout omits them from disk, so the read reported "file not found" and the check exited 1 against a valid working tree.
- Removed the sentence requiring active-user consent for `git status` and `git diff` from `AGENTS.md`, its synced copies, `README.md`, and `plan/HANDOFF.md.example`. It contradicted Rule 10, which requires verifying repository state before acting, and the hardened invocations it replaced were a denylist that `filter.<driver>.clean` defeats.
- Added `--no-show-signature` to the documented `git log` invocation in every prescriptive copy. Under `log.showSignature=true` the command spawned `gpg.program`, which the surrounding paragraph exists to forbid; only the CHANGELOG had described the safe form.
