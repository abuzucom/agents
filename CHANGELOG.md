...
- Added `scripts/check_conflict_markers.py` with `--staged` mode reading index blobs for pre-commit hooks, TOCTOU-safe single-fd file I/O via `_safe_read`, repo root resolution via `git rev-parse --show-toplevel`, generic-plus-configured marker pattern recognition, UTF-16/32 BOM decoding, and Markdown Setext heading context validation.
- Added `tests/test_check_conflict_markers.py`, 33 tests covering 2-way and diff3 conflict blocks, custom marker sizes including false-negative regression for size 10 and degenerate size 1, UTF-16 BOMs, symlinks, `_safe_read` unit tests, Setext headings, CLI from subdirectory, `--staged` mode, and exact wiring across CI, Makefile, and pre-commit.
